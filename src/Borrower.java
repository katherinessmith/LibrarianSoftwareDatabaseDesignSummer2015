...
public class Borrower
{
	...
	{
		Connection con = WrapDB.getConnection();

		if(con == null)
		{
			returnMessage = "Cannot connect to the database";
			return false;
		}
		...
		try
		{
			...
			ps.execute();
			returnMessage = "New borrower added";
			return true;
		}
		catch (Exception e)
		{
	...
	private int generateNewCardNumber() throws SQLException
	{
		...
		ResultSet rs = WrapDB.ExecuteSelectQuery(readCardNum);
		rs.next();
		int cardNumber = rs.getInt("card");
		WrapDB.ExecuteUpdateQuery(updateCardNum);
		return cardNumber;
	}
	...
	private boolean checkIfExists() throws SQLException
	{
		String query = "select count(card_no) as counter from borrower where fname = '" + fname + 
				...
				"' and phone = '" + phone + "';";
		ResultSet rs = WrapDB.ExecuteSelectQuery(query);
		rs.next();
	...
	public double[] getBorrowerStats()
	{
		try
		{
			...
			ResultSet rs1 = WrapDB.ExecuteSelectQuery(totalCheckOutBooks);
			rs1.next();
			...
			ResultSet rs2 = WrapDB.ExecuteSelectQuery(finePaidSoFarQuery);
			rs2.next();
			...
			ResultSet rs3 = WrapDB.ExecuteSelectQuery(checkedInFineQuery);
			rs3.next();
			...
			ResultSet rs4 = WrapDB.ExecuteSelectQuery(checkedOutFineQuery);
			rs4.next();
	...
	public void payFineAmount() throws SQLException
	{
		...
		ResultSet rs = WrapDB.ExecuteSelectQuery(getCheckInLoans);
		while (rs.next())
		{
			int loanId = rs.getInt("loans");
			String payFine = "update fines set paid = true where loan_id = " + loanId + ";";
			WrapDB.ExecuteUpdateQuery(payFine);
		}
		returnMessage = "Fine paid";
	}
}
