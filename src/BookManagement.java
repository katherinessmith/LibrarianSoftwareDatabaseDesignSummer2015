...
public class BookManagement
{
	...
	public static boolean canCheckOutBooks(int cardNumber)
	{
		...
		try
		{
			...
			{
				returnMessage = "Borrower already has 3 books checked out already\n";
				return false;
			}
			else
				return true;
	...
	private static void updateBookLoan(int loanId, String bookId, int branchId) throws SQLException
	{
		...
		WrapDB.ExecuteUpdateQuery(bookLoanUpdateQuery);
	}
	...
	private static boolean checkIfBooksAvailable(String bookId, String branchId) throws SQLException
	{
		...
		ResultSet rs = WrapDB.ExecuteSelectQuery(checkCopies);
		if(!rs.next())
		{
			returnMessage = "This branch is out of copies for this book, please check another branch";
			return false;
		}
		else {
			return true;
	...
	private static int readNewLoanId() throws SQLException
	{
		...
		ResultSet rs = WrapDB.ExecuteSelectQuery(readLoanId);
		rs.next();
	...
	private static void updateLoanId() throws SQLException
	{
		...
		WrapDB.ExecuteUpdateQuery(updateLoanId);
	}
