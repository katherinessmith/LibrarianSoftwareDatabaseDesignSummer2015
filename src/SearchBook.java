...
public class SearchBook {
	...
	public static DefaultTableModel returnLoanedBooks(BookLoanData bookloan) {
		...
		String query = "select l.card_no, l.loan_id, l.book_id, l.branch_id, b.title from "
				...
				+ "where l.book_id like '%"
				+ bookloan.getBookId()
				+ "%' and b.book_id = l.book_id and "
				+ "l.card_no like '%"
				+ bookloan.getCardNumber()
				+ "%' and isnull(l.date_in) and "
				+ "(br.fname like '%"
				+ bookloan.getBorrower()
				+ "%' OR br.lname like '%"
				+ bookloan.getBorrower()
				+ "%') and br.card_no = l.card_no;";

		ResultSet rs;
		try {
			rs = WrapDB.ExecuteSelectQuery(query);
			while (rs.next()) {
	...
	public static DefaultTableModel returnAvailability(
			SearchParas searchBook) {
		...
		String query = "select * from SearchAvailableBooks where book_id LIKE '%"
				+ searchBook.getBookId()
				+ "%' AND title LIKE '%"
				+ searchBook.getBookTitle()
				+ "%' AND authors LIKE '%"
				+ searchBook.getBookAuthor() + "%';";
